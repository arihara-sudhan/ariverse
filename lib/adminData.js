...
import { notifyNewsletterSubscribers } from './newsletter';
...
const defaultLinks = [
  ...
  { label: 'அரியின் கவிதைகள்', href: '/arichuvadi?category=kavithaigal', category: 'HOBBYAL' },
...
async function ensureInitialized({ background = false } = {}) {
  ...
  if (!initPromise) {
    ...
    initPromise = (async () => {
      ...
      await sql`
        UPDATE profile_links
        SET label = 'அரியின் கவிதைகள்', href = '/arichuvadi?category=kavithaigal'
      ...
      await sql`
        ...
        WHERE label IN ('Clay Play', 'அரியின் கவிதைகள்', 'Books Read', 'Book Reviews', 'Binomial Names', 'AriZone')
      `;
...
function newsletterPathByLabel(label) {
  if (label === 'Career' || label === 'Works' || label === 'Experience') return '/ari-career';
  if (label === 'Projects') return '/projects';
  if (label === 'Mini-Projects') return '/mini-projects';
  if (label === 'Experiments') return '/aris-xperiments';
  if (label === 'Guest Lectures') return '/guest-lectures';
  if (label === 'Book Reviews') return '/book-reviews';
  if (label === 'Clay Play') return '/clay-play';
  if (label === 'Books Read') return '/ari-read-books';
  if (label === 'அரியின் கவிதைகள்' || label === 'Ariyin Kavithaigal' || label === 'Kavithaigal') return '/arichuvadi?category=kavithaigal';
  return '';
}

async function notifyNewsletterForLabel({ label, title, summary }) {
  const itemUrl = newsletterPathByLabel(label);
  if (!itemUrl) return;
  try {
    ...
    // Newsletter delivery should never block content creation.
  }
}
...
export async function addLinkItem({ linkId, imageUrl, imageUrls, youtubeUrl, markdownText, kavithaiFrom, imageAlign, category, subcategory, companyLogoUrl, subtitle, dateText, bigDescription, projectTags }) {
  ...
  if (label === 'Mini-Projects') {
    ...
    const rows = await sql`
      ...
      RETURNING id, link_id AS "linkId", logo_url AS "imageUrl", embed_link AS "youtubeUrl",
                ...
                category, sort_order AS "sortOrder"
    `;
    await notifyNewsletterForLabel({
      label,
      title: rows[0]?.kavithaiFrom || kavithaiFrom || '',
  ...
  if (label === 'Projects') {
    ...
    await notifyNewsletterForLabel({
      label,
      title: rows[0]?.kavithaiFrom || kavithaiFrom || '',
      summary: bigDescription || markdownText || '',
    });
    return {
      ...rows[0],
  ...
  if (label === 'Guest Lectures') {
    ...
    await notifyNewsletterForLabel({
      label,
      title: guestRows[0]?.kavithaiFrom || kavithaiFrom || '',
      summary: markdownText || '',
    });
    return {
      ...guestRows[0],
  ...
  if (label === 'Book Reviews') {
    ...
    await notifyNewsletterForLabel({
      label,
      title: bookReviewRows[0]?.kavithaiFrom || kavithaiFrom || '',
      summary: markdownText || '',
    });
    return {
      ...bookReviewRows[0],
  ...
  if (label === 'Clay Play') {
    ...
    await notifyNewsletterForLabel({
      label,
      title: clayRows[0]?.kavithaiFrom || kavithaiFrom || '',
      summary: markdownText || '',
    });
    return {
      ...clayRows[0],
  ...
  if (label === 'Books Read') {
    ...
    await notifyNewsletterForLabel({
      label,
      title: bookRows[0]?.kavithaiFrom || kavithaiFrom || '',
      summary: markdownText || '',
    });
    return {
      ...bookRows[0],
  ...
  if (label === 'Career' || label === 'Works' || label === 'Experience') {
    ...
    await notifyNewsletterForLabel({
      label,
      title: rows[0]?.kavithaiFrom || kavithaiFrom || '',
      summary: [subtitle, dateText, markdownText].filter(Boolean).join(' - '),
    });
    return rows[0] || null;
  }
...
export async function listAvailableImages() {
  ...
  const [
    ...
    `,
    sql`
      SELECT image_url AS "imageUrl", company_logo_url AS "companyLogoUrl"
      FROM career_entries
      WHERE TRIM(COALESCE(image_url, '')) <> '' OR TRIM(COALESCE(company_logo_url, '')) <> ''
    `,
    sql`
      SELECT image_url AS "imageUrl"
      FROM experiments_entries
    ...
    `,
    sql`
      SELECT image_url AS "imageUrl", image_urls AS "imageUrlsRaw"
      FROM clay_play_entries
      WHERE TRIM(COALESCE(image_url, '')) <> '' OR TRIM(COALESCE(image_urls, '')) <> ''
    `,
    sql`
      SELECT image_url AS "imageUrl", image_urls AS "imageUrlsRaw"
      FROM guest_lectures_entries
    ...
    `,
    sql`
      SELECT page_image_urls AS "pageImageUrlsRaw"
      FROM resume_assets
      WHERE TRIM(COALESCE(page_image_urls::text, '')) <> '[]'
    `,
  ]);
  ...
  for (const row of heroRows) addAvailableImage(images, seen, row.imageUrl, 'Section Hero');
  for (const row of kavithaiRows) addAvailableImage(images, seen, row.imageUrl, 'Ariyin Kavithaigal');
  for (const row of booksReadRows) addAvailableImage(images, seen, row.imageUrl, 'Books Read');
  for (const row of shelfRows) addAvailableImage(images, seen, row.imageUrl, 'Shelf');
  for (const row of arisBooksRows) addAvailableImage(images, seen, row.coverUrl, 'My Books');
  for (const row of miniProjectRows) addAvailableImage(images, seen, row.logoUrl, 'Mini-Projects');
  for (const row of projectRows) addAvailableImage(images, seen, row.logoUrl, 'Projects');
  for (const row of careerRows) {
    addAvailableImage(images, seen, row.imageUrl, 'Career');
    addAvailableImage(images, seen, row.companyLogoUrl, 'Career Logo');
  }
  for (const row of experimentsRows) addAvailableImage(images, seen, row.imageUrl, 'Experiments');
  for (const row of clayPlayRows) {
    addAvailableImage(images, seen, row.imageUrl, 'Clay Play');
    addAvailableImageValues(images, seen, row.imageUrlsRaw, 'Clay Play');
  }
  for (const row of guestLectureRows) {
    addAvailableImage(images, seen, row.imageUrl, 'Guest Lectures');
    addAvailableImageValues(images, seen, row.imageUrlsRaw, 'Guest Lectures');
  }
  for (const row of bookReviewRows) {
    addAvailableImage(images, seen, row.imageUrl, 'Book Reviews');
    addAvailableImageValues(images, seen, row.imageUrlsRaw, 'Book Reviews');
  }
  for (const row of resumeRows) {
    addAvailableImageValues(images, seen, row.pageImageUrlsRaw, 'Resume');
  }

  return images.sort((left, right) => left.url.localeCompare(right.url));
}
...
export async function addContentEntryLike({ sectionKey, entryId, name = '', count = 1 }) {
  ...
  for (let index = 0; index < likeCount; index += 1) {
    await sql`
      ...
      VALUES (${safeSection}, ${resolvedEntryId}, ${safeName})
    `;
  }
