...
let initPromise;
const DB_INIT_VERSION = '2026-05-22-fast-init-v1';
const LINK_FIX_VERSION = '2026-05-22-link-fixes-v3';
  ...
  if (!initPromise) {
    initPromise = (async () => {
      ...
      await sql`
        ...
        WHERE label IN ('Works', 'Projects', 'Skillset', 'Resume', 'Mini-Projects')
      `;
...
export async function getLinkItemById(id) {
  await ensureInitialized();
  const resolved = await resolveItemTableById(id);
