...
import { HOME_HERO_IMAGE_URL, normalizeStorageValue, toPublicStorageUrl } from './storage';
...
const KAVITHAI_LINK_LABELS = new Set(['My Poems', 'à®…à®°à®¿à®¯à®¿à®©à¯ à®•à®µà®¿à®¤à¯ˆà®•à®³à¯', 'Ariyin Kavithaigal', 'Kavithaigal']);

function isKavithaiLinkLabel(value) {
  return KAVITHAI_LINK_LABELS.has(String(value || '').trim());
}
...
function normalizeSkillSvgList(value) {
  if (Array.isArray(value)) {
    return value.map((item) => (typeof item === 'string' ? item.trim() : '')).filter(Boolean);
  }
  if (typeof value === 'string') {
...
function normalizeProfileLinkRow(row) {
  if (!row || typeof row !== 'object') return row;
  if (String(row.label || '').trim() === 'Arichuvadu' || String(row.label || '').trim() === 'Arichuvadi (Blog)') {
    return {
      ...row,
      label: 'Arichuvadi',
      href: '/arichuvadi',
  ...
  if (String(row.label || '').trim() === 'AriZone (Blog)') {
    return {
      ...row,
      label: 'AriZone',
      href: '/arizone',
  ...
  if (String(row.label || '').trim() !== 'Career') return row;
  if (String(row.href || '').trim() === '/ari-career') return row;
  return {
    ...row,
    href: '/ari-career',
  };
...
function normalizeSkillsetEntryRow(row, index = 0) {
  ...
  return {
    ...
    skillsSvgList: normalizeSkillSvgList(row.skillsSvgListRaw ?? row.skillsSvgList),
...
const defaultLinks = [
  ...
  { label: 'Career', href: '/ari-career', category: 'PROFESSIONAL' },
  { label: 'Portfolio', href: '/ari-folio', category: 'PROFESSIONAL' },
  { label: 'Experiments', href: '/aris-xperiments', category: 'PASSIONAL' },
  { label: 'ARI XPands', href: '/ari-xpands', category: 'PASSIONAL' },
  ...
  { label: 'AriZone', href: '/arizone', category: 'HOBBYAL' },
  { label: 'Arichuvadi', href: '/arichuvadi', category: 'HOBBYAL' },
  { label: 'My Books', href: '/aris-books', category: 'PASSIONAL' },
  { label: 'Shelf', href: '/aris-shelf', category: 'PASSIONAL' },
  ...
  { label: 'My Poems', href: '/arichuvadi?category=kavithaigal', category: 'HOBBYAL' },
...
const DEFAULT_SKILLSET_ENTRIES = [
  {
    ...
    skillsSvgList: ['python-core', 'anaconda', 'numpy', 'jupyter'],
  },
  {
  ...
  },
  {
    ...
    skillsSvgList: ['opencv'],
  },
  {
    skillname: 'Programming Foundations',
    ...
    skillsSvgList: ['c-programming', 'core-java', 'vanilla-js', 'typescript'],
  },
  {
    ...
    skillsSvgList: ['html', 'css', 'bootstrap', 'tailwind-css', 'reactjs', 'redux'],
  },
  {
    ...
    skillsSvgList: ['node-js', 'expressjs', 'fastapi', 'flask', 'django'],
  },
  {
    skillname: 'Data & Persistence',
    title: 'Storage layer',
    description:
      'Working with relational and document databases to support application state, querying, and structured data management.',
    skillsSvgList: ['mysql', 'mongodb'],
  },
  {
    skillname: 'Cloud, DevOps & Collaboration',
    title: 'Delivery',
    description:
      'Versioning, containerization, cloud deployment, and team workflows that keep software reliable and shippable.',
    skillsSvgList: ['docker', 'git', 'github', 'gitlab', 'aws', 'azure'],
  },
  {
    skillname: 'Automation & Creative Tech',
    ...
    skillsSvgList: ['uipath', 'blender', 'davinci-resolve'],
  },
];

const connectionString = process.env.DATABASE_URL;
function createUnavailableSql() {
  return () => {
...
async function ensureShelfTable() {
  await sql`
  ...
  `;
  await sql`ALTER TABLE shelf_entries ADD COLUMN IF NOT EXISTS subname TEXT NOT NULL DEFAULT ''`;
}

async function ensureResumeAssetsTable() {
  await sql`
    CREATE TABLE IF NOT EXISTS resume_assets (
      link_id INTEGER PRIMARY KEY REFERENCES profile_links(id) ON DELETE CASCADE,
  ...
  `;
  await sql`ALTER TABLE resume_assets ADD COLUMN IF NOT EXISTS resume_text TEXT NOT NULL DEFAULT ''`;
  await sql`ALTER TABLE resume_assets ADD COLUMN IF NOT EXISTS pdf_url TEXT NOT NULL DEFAULT ''`;
  await sql`ALTER TABLE resume_assets ADD COLUMN IF NOT EXISTS page_image_urls JSONB NOT NULL DEFAULT '[]'::jsonb`;
  await sql`ALTER TABLE resume_assets ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT now()`;
}

async function ensureSkillsetTable() {
  await sql`
    CREATE TABLE IF NOT EXISTS skillset_entries (
      id SERIAL PRIMARY KEY,
      skillname TEXT NOT NULL DEFAULT '',
      title TEXT NOT NULL DEFAULT '',
      description TEXT NOT NULL DEFAULT '',
      skills_svg_list JSONB NOT NULL DEFAULT '[]'::jsonb,
      sort_order INTEGER NOT NULL DEFAULT 0,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
  `;
  await sql`ALTER TABLE skillset_entries ADD COLUMN IF NOT EXISTS skillname TEXT NOT NULL DEFAULT ''`;
  await sql`ALTER TABLE skillset_entries ADD COLUMN IF NOT EXISTS title TEXT NOT NULL DEFAULT ''`;
  await sql`ALTER TABLE skillset_entries ADD COLUMN IF NOT EXISTS description TEXT NOT NULL DEFAULT ''`;
  await sql`ALTER TABLE skillset_entries ADD COLUMN IF NOT EXISTS skills_svg_list JSONB NOT NULL DEFAULT '[]'::jsonb`;
  await sql`ALTER TABLE skillset_entries ADD COLUMN IF NOT EXISTS sort_order INTEGER NOT NULL DEFAULT 0`;
  await sql`ALTER TABLE skillset_entries ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT now()`;
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_skillset_entries_skillname ON skillset_entries(skillname)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_skillset_entries_sort_order ON skillset_entries(sort_order ASC, id ASC)`;

  const countRows = await sql`SELECT COUNT(*)::int AS count FROM skillset_entries`;
  if ((countRows[0]?.count || 0) > 0) return;

  for (let index = 0; index < DEFAULT_SKILLSET_ENTRIES.length; index += 1) {
    const entry = DEFAULT_SKILLSET_ENTRIES[index];
    await sql`
      INSERT INTO skillset_entries (skillname, title, description, skills_svg_list, sort_order)
      VALUES (${entry.skillname}, ${entry.title}, ${entry.description}, ${JSON.stringify(entry.skillsSvgList)}::jsonb, ${
        index + 1
      })
    `;
  }
...
const DB_INIT_VERSION = '2026-06-13-resume-text-v3';
const LINK_FIX_VERSION = '2026-09-05-portfolio-link-v9';
...
const EXPERIMENTS_COLLAGE_HERO_FIX_VERSION = '2026-06-17-collage-helps-hero-v1';
...
const SHELF_IMAGE_OVERRIDES = new Map([
  [3, 'https://qbghhenrxoupaykgnxyj.supabase.co/storage/v1/object/public/ariverse/ari-shelf/facial-paint1-aspc.webp'],
  [23, 'https://qbghhenrxoupaykgnxyj.supabase.co/storage/v1/object/public/ariverse/ari-shelf/lions-club-xii.webp'],
  [25, 'https://qbghhenrxoupaykgnxyj.supabase.co/storage/v1/object/public/ariverse/ari-shelf/yuva-kala.webp'],
]);
...
const sql = connectionString ? neon(connectionString) : createUnavailableSql();
let initPromise;
let initAttemptId = 0;
let activeInitAttemptId = 0;
let timedOutInitAttemptId = 0;
let dbReady = false;
...
let dbRetryAfter = 0;
let projectCommentsTablesPromise = null;
let contentCommentsTablesPromise = null;
let contentEntryLikesTablesPromise = null;
let testimonialsTablesPromise = null;
let featureImagesTablesPromise = null;
let kavithaiLinkCache = null;
let kavithaiLinkCacheExpiresAt = 0;

function invalidateProfileLinksCache() {
  profileLinksCache = { all: null, visible: null, allExpiresAt: 0, visibleExpiresAt: 0 };
  kavithaiLinkCache = null;
  kavithaiLinkCacheExpiresAt = 0;
}
...
async function ensureInitialized({ background = false } = {}) {
  if (dbUnavailable) {
  ...
  if (dbReady) return;
  if (!initPromise) {
    activeInitAttemptId = ++initAttemptId;
    timedOutInitAttemptId = 0;
    initPromise = (async () => {
      await sql`
      ...
      const metaRows = await sql`
        ...
        WHERE key IN (
          'db_init_version',
          'link_fix_version',
          'career_asset_path_fix_version',
          'hero_asset_path_fix_version',
          'experiments_collage_hero_fix_version',
          'mini_project_category_rename_version',
          'mini_project_category_normalize_version'
        )
      `;
      const metaState = Object.fromEntries(metaRows.map((row) => [row.key, row.value]));
      const initVersion = metaState.db_init_version || '';
      const linkFixVersion = metaState.link_fix_version || '';
      const careerAssetPathFixVersion = metaState.career_asset_path_fix_version || '';
      const heroAssetPathFixVersion = metaState.hero_asset_path_fix_version || '';
      const experimentsCollageHeroFixVersion = metaState.experiments_collage_hero_fix_version || '';
      const miniProjectCategoryRenameVersion = metaState.mini_project_category_rename_version || '';
      const miniProjectCategoryNormalizeVersion = metaState.mini_project_category_normalize_version || '';
      if (
        initVersion === DB_INIT_VERSION &&
        linkFixVersion === LINK_FIX_VERSION &&
        careerAssetPathFixVersion === CAREER_ASSET_PATH_FIX_VERSION &&
        heroAssetPathFixVersion === HERO_ASSET_PATH_FIX_VERSION &&
        experimentsCollageHeroFixVersion === EXPERIMENTS_COLLAGE_HERO_FIX_VERSION &&
        miniProjectCategoryRenameVersion === MINI_PROJECT_CATEGORY_RENAME_VERSION &&
        miniProjectCategoryNormalizeVersion === MINI_PROJECT_CATEGORY_NORMALIZE_VERSION
      ) {
        dbReady = true;
        dbUnavailable = false;
        dbErrorLogged = false;
      ...
      if (linkFixVersion !== LINK_FIX_VERSION) {
        ...
        await sql`UPDATE profile_links SET href = '/ari-career' WHERE label = 'Career'`;
        await sql`
        ...
        await sql`UPDATE profile_links SET href = '/ari-folio' WHERE label = 'Portfolio'`;
        const existingPortfolio = await sql`SELECT id FROM profile_links WHERE label = 'Portfolio' LIMIT 1`;
        if (!existingPortfolio[0]) {
          await sql`
            INSERT INTO profile_links (label, href, category, sort_order, is_hidden)
            VALUES ('Portfolio', '/ari-folio', 'PROFESSIONAL', 2, 0)
          `;
        }
        ...
        await sql`UPDATE profile_links SET label = 'My Poems' WHERE label IN ('Ariyin Kavithaigal', 'Kavithaigal', 'அரியின் கவிதைகள்')`;
        await sql`
      ...
      if (careerAssetPathFixVersion !== CAREER_ASSET_PATH_FIX_VERSION) {
        await sql`
      ...
      if (heroAssetPathFixVersion !== HERO_ASSET_PATH_FIX_VERSION) {
        await sql`
      ...
      if (experimentsCollageHeroFixVersion !== EXPERIMENTS_COLLAGE_HERO_FIX_VERSION) {
        await sql`
          UPDATE experiments_entries
          SET image_url = ${toPublicStorageUrl('ari-xperiments/collage-helps/collage-helps-hero.webp')}
          WHERE entry_title = 'What If We Fed Collages Instead of Single Images?'
            AND image_url IS DISTINCT FROM ${toPublicStorageUrl('ari-xperiments/collage-helps/collage-helps-hero.webp')}
        `;
        await sql`
          INSERT INTO ariverse_meta (key, value, updated_at)
          VALUES ('experiments_collage_hero_fix_version', ${EXPERIMENTS_COLLAGE_HERO_FIX_VERSION}, now())
          ON CONFLICT (key)
          DO UPDATE SET value = EXCLUDED.value, updated_at = now()
        `;
      }
      if (miniProjectCategoryRenameVersion !== MINI_PROJECT_CATEGORY_RENAME_VERSION) {
        await sql`
      ...
      if (miniProjectCategoryNormalizeVersion !== MINI_PROJECT_CATEGORY_NORMALIZE_VERSION) {
        await sql`
          UPDATE mini_projects_entries
          SET category = CASE
      ...
      await ensureShelfTable();
      await ensureResumeAssetsTable();
      await ensureSkillsetTable();

      if (initVersion === DB_INIT_VERSION) {
        return;
      }
      ...
      `;
      await sql`
        CREATE TABLE IF NOT EXISTS book_reviews_entries (
          id SERIAL PRIMARY KEY,
          link_id INTEGER NOT NULL REFERENCES profile_links(id) ON DELETE CASCADE,
          image_url TEXT NOT NULL DEFAULT '',
          image_urls TEXT NOT NULL DEFAULT '[]',
          youtube_url TEXT NOT NULL DEFAULT '',
          category TEXT NOT NULL DEFAULT 'TAMIL',
          markdown_text TEXT NOT NULL DEFAULT '',
          entry_title TEXT NOT NULL DEFAULT '',
          image_align TEXT NOT NULL DEFAULT 'left',
          sort_order INTEGER NOT NULL,
      ...
      `;
      await sql`ALTER TABLE book_reviews_entries ADD COLUMN IF NOT EXISTS youtube_url TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE book_reviews_entries ADD COLUMN IF NOT EXISTS category TEXT NOT NULL DEFAULT 'TAMIL'`;
      await sql`
        UPDATE book_reviews_entries
        SET category = CASE
          WHEN UPPER(TRIM(COALESCE(category, ''))) = 'ENGLISH' THEN 'ENGLISH'
          ELSE 'TAMIL'
        END
      `;
      await sql`
      ...
      `;
      await sql`
        CREATE TABLE IF NOT EXISTS book_reviews_entry_likes (
          id SERIAL PRIMARY KEY,
          entry_id INTEGER NOT NULL REFERENCES book_reviews_entries(id) ON DELETE CASCADE,
          person_name TEXT NOT NULL,
          created_at TIMESTAMPTZ NOT NULL DEFAULT now()
        );
      `;
      ...
      `;
      await sql`
        CREATE INDEX IF NOT EXISTS idx_book_reviews_entry_likes_entry_id
          ON book_reviews_entry_likes(entry_id);
      `;
      await ensureProjectsTable();
      await sql`
      ...
      `;
      await sql`ALTER TABLE project_comments ADD COLUMN IF NOT EXISTS avatar_url TEXT NOT NULL DEFAULT ''`;
      await sql`
      ...
      await sql`
        UPDATE profile_links
        SET label = 'My Poems', href = '/arichuvadi?category=kavithaigal'
        WHERE label IN ('Kavithaigal', 'Ariyin Kavithaigal', 'அரியின் கவிதைகள்')
      `;
      ...
      `;
      const existingArichuvadi = await sql`SELECT id FROM profile_links WHERE label IN ('Arichuvadi', 'Arichuvadi (Blog)') LIMIT 1`;
      if (!existingArichuvadi[0]) {
        const maxArichuvadi = await sql`SELECT COALESCE(MAX(sort_order), 0)::int AS max_order FROM profile_links`;
        await sql`
          INSERT INTO profile_links (label, href, category, sort_order, is_hidden)
          VALUES ('Arichuvadi', '/arichuvadi', 'HOBBYAL', ${(maxArichuvadi[0]?.max_order || 0) + 1}, 0)
        `;
      } else {
        await sql`
          UPDATE profile_links
          SET label = 'Arichuvadi', href = '/arichuvadi', category = 'HOBBYAL'
          WHERE id = ${existingArichuvadi[0].id}
        `;
      }
      await sql`
        UPDATE profile_links
      ...
      await sql`UPDATE profile_links SET href = '/aris-shelf' WHERE label = 'Shelf'`;
      await sql`
        UPDATE profile_links
        SET category = 'PROFESSIONAL'
        WHERE label IN ('Career', 'Portfolio', 'Projects', 'Skillset', 'Resume', 'Mini-Projects')
      `;

      const existingPortfolio = await sql`SELECT id FROM profile_links WHERE label = 'Portfolio' LIMIT 1`;
      if (!existingPortfolio[0]) {
        const maxPortfolio = await sql`SELECT COALESCE(MAX(sort_order), 0)::int AS max_order FROM profile_links`;
        await sql`
          INSERT INTO profile_links (label, href, category, sort_order, is_hidden)
          VALUES ('Portfolio', '/ari-folio', 'PROFESSIONAL', ${(maxPortfolio[0]?.max_order || 0) + 1}, 0)
        `;
      }
      ...
      await sql`
        ...
        WHERE label IN ('Experiments', 'My Books', 'Shelf', 'Thirukkural', 'For AI', 'Guest Lectures', 'AI with ARI (YouTube)')
      `;
      ...
      const existingShelf = await sql`SELECT id FROM profile_links WHERE label = 'Shelf' LIMIT 1`;
      if (!existingShelf[0]) {
        const maxShelf = await sql`SELECT COALESCE(MAX(sort_order), 0)::int AS max_order FROM profile_links`;
        await sql`
          INSERT INTO profile_links (label, href, category, sort_order, is_hidden)
          VALUES ('Shelf', '/aris-shelf', 'PASSIONAL', ${(maxShelf[0]?.max_order || 0) + 1}, 0)
        `;
      }
      ...
      await sql`
        ...
        WHERE label IN ('Clay Play', 'My Poems', 'அரியின் கவிதைகள்', 'Books Read', 'Book Reviews', 'Binomial Names', 'AriZone')
      `;
      ...
      if (legacyLinkItems[0]?.name) {
        await sql`
          ...
          WHERE pl.label IN ('My Poems', 'அரியின் கவிதைகள்', 'Ariyin Kavithaigal', 'Kavithaigal')
        `;
      ...
      initPromise = null;
      dbReady = false;
      if (activeInitAttemptId === timedOutInitAttemptId) {
        timedOutInitAttemptId = 0;
      }
  ...
  if (background && !dbReady) {
    markDbInitializing(new Error('Database initialization still in progress'));
    return;
  }

  if (initPromise && activeInitAttemptId === timedOutInitAttemptId) {
    markDbInitializing(new Error('Database initialization still in progress'));
    return;
  }
  ...
  try {
    ...
    if (error?.code === DB_INIT_TIMEOUT_ERROR_CODE) {
      timedOutInitAttemptId = activeInitAttemptId;
...
async function ensurePublicInitialized() {
  // Public pages should wait for the DB bootstrap instead of caching an empty
  // first render while initialization is still running in the background.
...
async function resolveKavithaiLink() {
  const now = Date.now();
  if (kavithaiLinkCache && kavithaiLinkCacheExpiresAt > now) return kavithaiLinkCache;
  const resolved =
    (await getProfileLinkByHref('/arichuvadi?category=kavithaigal')) ||
    (await getProfileLinkByLabel('My Poems')) ||
    ...
    null;
  kavithaiLinkCache = resolved;
  kavithaiLinkCacheExpiresAt = now + PROFILE_LINKS_CACHE_TTL_MS;
  return resolved;
}
...
function tableByLabel(label) {
  ...
  if (isKavithaiLinkLabel(label)) return 'ariyin_kavithaigal';
  return null;
}
  ...
  const rows = expectedLinkId
    ? await sql`
        SELECT * FROM (
          ...
          UNION ALL
          SELECT id, link_id AS "linkId", 'book_reviews_entries'::text AS table_name FROM book_reviews_entries WHERE id = ${id} AND link_id = ${expectedLinkId}
          UNION ALL
          ...
          UNION ALL
          SELECT id, link_id AS "linkId", 'shelf_entries'::text AS table_name FROM shelf_entries WHERE id = ${id} AND link_id = ${expectedLinkId}
          UNION ALL
    ...
    : await sql`
        SELECT * FROM (
          ...
          UNION ALL
          SELECT id, link_id AS "linkId", 'book_reviews_entries'::text AS table_name FROM book_reviews_entries WHERE id = ${id}
          UNION ALL
          ...
          UNION ALL
          SELECT id, link_id AS "linkId", 'shelf_entries'::text AS table_name FROM shelf_entries WHERE id = ${id}
          UNION ALL
...
export async function listProfileLinks() {
  ...
  const normalizedRows = rows.map((row) => normalizeProfileLinkRow(row));
  profileLinksCache = {
    ...profileLinksCache,
    all: normalizedRows,
...
export async function getProfileLinkById(id) {
  ...
  return normalizeProfileLinkRow(rows[0] || null);
}
...
export async function getProfileLinkByHref(href) {
  await ensurePublicInitialized();
  if (dbUnavailable) return null;
  const rows = await sql`SELECT id, label, href, category, sort_order AS "sortOrder", is_hidden AS "isHidden" FROM profile_links WHERE href = ${href} LIMIT 1`;
  return normalizeProfileLinkRow(rows[0] || null);
}
...
export async function listLinkItems(linkId) {
  ...
  if (label === 'Mini-Projects') {
    ...
    `;
    return rows.map((row) => normalizeReturnedRow(row));
  }
  if (label === 'Projects') {
    await ensureProjectsTable();
    const rows = await sql`
    ...
    return rows.map((row) => normalizeReturnedRow({ ...row, projectTags: tagsByEntry.get(row.id) || [] }));
  }
  ...
  if (label === 'Shelf') {
    await ensureShelfTable();
    const rows = await sql`
      SELECT id, link_id AS "linkId", image_url AS "imageUrl", entry_name AS "kavithaiFrom",
             subname AS "markdownText", 'left'::text AS "imageAlign", sort_order AS "sortOrder"
      FROM shelf_entries
      WHERE link_id = ${linkId}
      ORDER BY sort_order ASC, id ASC
    `;
  ...
  if (label === 'Binomial Names') {
    return sql`
      ...
      WHERE link_id = ${linkId}
      ORDER BY sort_order ASC, id ASC
    `.then((rows) => rows.map((row) => normalizeReturnedRow(row)));
  }

  if (isKavithaiLinkLabel(label)) {
    return sql`
      SELECT id, link_id AS "linkId", image_url AS "imageUrl", ''::text AS "youtubeUrl",
  ...
  if (label === 'Book Reviews') {
    const rows = await sql`
      SELECT id, link_id AS "linkId", image_url AS "imageUrl", youtube_url AS "youtubeUrl",
             image_urls AS "imageUrlsRaw", category, markdown_text AS "markdownText",
             entry_title AS "kavithaiFrom", image_align AS "imageAlign",
             sort_order AS "sortOrder"
      FROM book_reviews_entries
      WHERE link_id = ${linkId}
      ORDER BY sort_order ASC, id ASC
    `;
    return rows.map((row) => ({
...
export async function getLinkItemById(id, linkId) {
  ...
  if (resolved.table === 'guest_lectures_entries') {
    ...
    if (!rows[0]) return null;
    return {
      ...normalizeReturnedRow(rows[0]),
      imageUrls: parseImageUrls(rows[0].imageUrlsRaw, rows[0].imageUrl),
    };
  }
  if (resolved.table === 'book_reviews_entries') {
    const rows = await sql`
      SELECT id, link_id AS "linkId", image_url AS "imageUrl", youtube_url AS "youtubeUrl",
             image_urls AS "imageUrlsRaw", category, markdown_text AS "markdownText",
             entry_title AS "kavithaiFrom", image_align AS "imageAlign",
  ...
  if (resolved.table === 'shelf_entries') {
    const rows = await sql`
      SELECT id, link_id AS "linkId", image_url AS "imageUrl", entry_name AS "kavithaiFrom",
             subname AS "markdownText", 'left'::text AS "imageAlign",
             sort_order AS "sortOrder"
      FROM shelf_entries
      WHERE id = ${id}
      LIMIT 1
    `;
    return rows[0] ? normalizeReturnedRow(rows[0]) : null;
  }
...
export async function listKavithaiEntries() {
  await ensurePublicInitialized();
  const kavithaiLink = await resolveKavithaiLink();
...
export async function listKavithaiSummaries() {
  await ensurePublicInitialized();
  const kavithaiLink = await resolveKavithaiLink();
  if (!kavithaiLink) return [];

  return sql`
    SELECT id, kavithai_name AS "kavithaiName", image_url AS "imageUrl", sort_order AS "sortOrder"
    FROM ariyin_kavithaigal
    WHERE link_id = ${kavithaiLink.id} AND TRIM(COALESCE(kavithai_name, '')) <> ''
    ORDER BY sort_order ASC, id ASC
  `.then((rows) => rows.map((row) => normalizeReturnedRow(row)));
...
export async function getKavithaiEntryById(id) {
  await ensurePublicInitialized();
  if (!Number.isInteger(Number(id)) || Number(id) <= 0) return null;
  const kavithaiLink = await resolveKavithaiLink();
  if (!kavithaiLink) return null;

  const rows = await sql`
    SELECT id, kavithai_name AS "kavithaiName", image_url AS "imageUrl", markdown_text AS "markdownText", sort_order AS "sortOrder"
    FROM ariyin_kavithaigal
    WHERE id = ${Number(id)} AND link_id = ${kavithaiLink.id}
    LIMIT 1
  `;
  return rows[0] ? normalizeReturnedRow(rows[0]) : null;
}
...
export async function addLinkItem({ linkId, imageUrl, imageUrls, youtubeUrl, markdownText, kavithaiFrom, imageAlign, category, subcategory, companyLogoUrl, subtitle, dateText, bigDescription, projectTags }) {
  ...
  if (label === 'Shelf') {
    await ensureShelfTable();
    const maxShelf = await sql`SELECT COALESCE(MAX(sort_order), 0)::int AS max_order FROM shelf_entries WHERE link_id = ${linkId}`;
    const rows = await sql`
      INSERT INTO shelf_entries (link_id, image_url, entry_name, subname, sort_order)
      VALUES (${linkId}, ${imageUrl || ''}, ${kavithaiFrom || ''}, ${markdownText || ''}, ${(maxShelf[0]?.max_order || 0) + 1})
      RETURNING id, link_id AS "linkId", image_url AS "imageUrl", entry_name AS "kavithaiFrom",
                subname AS "markdownText", 'left'::text AS "imageAlign", sort_order AS "sortOrder"
    `;
    return rows[0] || null;
  }
  ...
  if (label === 'Book Reviews') {
    const maxBookReviews = await sql`SELECT COALESCE(MAX(sort_order), 0)::int AS max_order FROM book_reviews_entries WHERE link_id = ${linkId}`;
    const align = imageAlign === 'right' ? 'right' : 'left';
    const normalizedUrls = normalizeImageUrls(imageUrls);
    const primaryImage = normalizedUrls[0] || imageUrl || '';
    const normalizedCategory = normalizeBookCategory(category);
    const bookReviewRows = await sql`
      INSERT INTO book_reviews_entries (link_id, image_url, image_urls, youtube_url, category, markdown_text, entry_title, image_align, sort_order)
      VALUES (${linkId}, ${primaryImage}, ${JSON.stringify(normalizedUrls)}, ${youtubeUrl || ''}, ${normalizedCategory}, ${markdownText || ''}, ${kavithaiFrom || ''}, ${align}, ${(maxBookReviews[0]?.max_order || 0) + 1})
      RETURNING id, link_id AS "linkId", image_url AS "imageUrl", youtube_url AS "youtubeUrl", category,
                image_urls AS "imageUrlsRaw", markdown_text AS "markdownText",
                entry_title AS "kavithaiFrom", image_align AS "imageAlign",
                sort_order AS "sortOrder"
    `;
    if (!bookReviewRows[0]) return null;
    return {
      ...bookReviewRows[0],
      imageUrls: parseImageUrls(bookReviewRows[0].imageUrlsRaw, bookReviewRows[0].imageUrl),
      category: normalizeBookCategory(bookReviewRows[0].category),
    };
  }
  if (label === 'Clay Play') {
...
export async function updateLinkItem({ id, linkId, imageUrl, imageUrls, youtubeUrl, markdownText, kavithaiFrom, imageAlign, category, subcategory, companyLogoUrl, subtitle, dateText, bigDescription, projectTags }) {
  ...
  if (resolved.table === 'shelf_entries') {
    await sql`UPDATE shelf_entries SET image_url = ${imageUrl || ''}, entry_name = ${kavithaiFrom || ''}, subname = ${markdownText || ''} WHERE id = ${id} AND link_id = ${resolved.linkId}`;
    return;
  }
  if (resolved.table === 'projects_entries') {
  ...
  if (resolved.table === 'book_reviews_entries') {
    const normalizedCategory = normalizeBookCategory(category);
    await sql`UPDATE book_reviews_entries SET image_url = ${primaryImage}, image_urls = ${JSON.stringify(normalizedUrls)}, youtube_url = ${youtubeUrl || ''}, category = ${normalizedCategory}, markdown_text = ${markdownText || ''}, entry_title = ${kavithaiFrom || ''}, image_align = ${align} WHERE id = ${id} AND link_id = ${resolved.linkId}`;
    return;
  }
}
...
export async function deleteLinkItem(id, linkId) {
  ...
  } else if (resolved.table === 'shelf_entries') {
    await sql`DELETE FROM shelf_entries WHERE id = ${id} AND link_id = ${resolved.linkId}`;
  ...
  } else if (resolved.table === 'book_reviews_entries') {
    await sql`DELETE FROM book_reviews_entries WHERE id = ${id} AND link_id = ${resolved.linkId}`;
...
export async function getProjectEntryBySlug(slug) {
  await ensurePublicInitialized();
  if (dbUnavailable) return null;
  ...
  const rows = await sql`
    ...
    WHERE link_id = ${link.id}
      AND trim(both '-' from regexp_replace(lower(title), '[^a-z0-9]+', '-', 'g')) = ${safeSlug}
    ORDER BY sort_order ASC, id ASC
    LIMIT 1
  `;
  if (!rows[0]) return null;
  ...
  const tagRows = await sql`
    ...
    FROM project_entry_tags pet
    JOIN project_tags pt ON pt.id = pet.tag_id
    WHERE pet.project_entry_id = ${rows[0].id}
    ORDER BY pt.name ASC
  `;
  const item = normalizeReturnedRow({
    ...rows[0],
    projectTags: tagRows.map((row) => row.name),
  });
...
export async function listExperimentsEntries() {
  ...
  const items = await listLinkItems(link.id);
  if (!Array.isArray(items) || items.length === 0) return [];
  const fallbackDetailsByTitle = new Map(
    fallbackAriTrials.map((item) => [String(item?.title || '').trim(), String(item?.detailText || '').trim()]),
  );
  return items.map((item) => ({
    id: item.id,
    title: item.kavithaiFrom || '',
    description: item.markdownText || '',
    fullDescription: item.bigDescription || fallbackDetailsByTitle.get(String(item.kavithaiFrom || '').trim()) || item.markdownText || '',
    imageUrl: item.imageUrl || '',
    readMoreUrl: `/aris-xperiments?id=${item.id}`,
  }));
}
...
export async function getSkillsetEntries() {
  await ensurePublicInitialized();
  if (dbUnavailable) {
    return DEFAULT_SKILLSET_ENTRIES.map((row, index) => normalizeSkillsetEntryRow(row, index));
  }
  ...
  const rows = await sql`
    SELECT
      ...
      title,
      description,
      skills_svg_list AS "skillsSvgListRaw",
      sort_order AS "sortOrder"
    FROM skillset_entries
    ORDER BY sort_order ASC, id ASC
  `;
  ...
  return rows.map((row, index) => normalizeSkillsetEntryRow(row, index));
}
...
export async function listShelfEntries() {
  await ensurePublicInitialized();
  const link = await getProfileLinkByLabel('Shelf');
  if (!link) return [];
  await ensureShelfTable();
  const rows = await sql`
    SELECT id, image_url AS "imageUrl", entry_name AS name, subname, sort_order AS "sortOrder"
    FROM shelf_entries
    WHERE link_id = ${link.id}
    ORDER BY sort_order ASC, id ASC
  `;
  return rows.map((row) => ({
    ...
    name: row.name || '',
    subname: row.subname || '',
    imageUrl: SHELF_IMAGE_OVERRIDES.get(row.id) || row.imageUrl || '',
  }));
}
...
export async function getSectionHero(linkId, fallbackHeading = '') {
  ...
  const rows = await sql`
    ...
    LIMIT 1
  `;
  if (rows[0]) return normalizeReturnedRow(rows[0]);
  return {
    linkId,
    heading: fallbackHeading || '',
    description: '',
...
export async function getResumeAssets(linkId) {
  await ensurePublicInitialized();
  if (!Number.isInteger(Number(linkId)) || Number(linkId) <= 0) return null;
  const rows = await sql`
    SELECT link_id AS "linkId", resume_text AS "resumeText", pdf_url AS "pdfUrl", page_image_urls AS "pageImageUrlsRaw", updated_at AS "updatedAt"
    FROM resume_assets
    WHERE link_id = ${Number(linkId)}
    LIMIT 1
  `;
  if (!rows[0]) return null;
  return normalizeReturnedRow({
    ...rows[0],
    resumeText: typeof rows[0].resumeText === 'string' ? rows[0].resumeText : '',
    pageImageUrls: normalizeImageUrls(rows[0].pageImageUrlsRaw),
    updatedAt:
      rows[0].updatedAt instanceof Date
        ? rows[0].updatedAt.toISOString()
        : typeof rows[0].updatedAt === 'string'
          ? rows[0].updatedAt
          : '',
  });
}

export async function upsertResumeAssets({ linkId, resumeText, pdfUrl, pageImageUrls }) {
  await ensureInitialized();
  const normalizedImages = normalizeImageUrls(pageImageUrls);
  const rows = await sql`
    INSERT INTO resume_assets (link_id, resume_text, pdf_url, page_image_urls, updated_at)
    VALUES (${linkId}, ${resumeText || ''}, ${pdfUrl || ''}, ${JSON.stringify(normalizedImages)}, now())
    ON CONFLICT (link_id)
    DO UPDATE SET
      resume_text = EXCLUDED.resume_text,
      pdf_url = EXCLUDED.pdf_url,
      page_image_urls = EXCLUDED.page_image_urls,
      updated_at = now()
    RETURNING link_id AS "linkId", resume_text AS "resumeText", pdf_url AS "pdfUrl", page_image_urls AS "pageImageUrlsRaw", updated_at AS "updatedAt"
  `;
  return normalizeReturnedRow({
    ...rows[0],
    resumeText: typeof rows[0].resumeText === 'string' ? rows[0].resumeText : '',
...
function normalizePublicImageUrl(url) {
  ...
  if (!input) return '';
  if (input.includes('blob.vercel-storage.com') || input.startsWith('/')) {
...
function normalizeReturnedRow(row) {
  return normalizeStorageValue(row);
}

function addAvailableImage(images, seen, url, source) {
...
function addAvailableImageValues(images, seen, values, source) {
  for (const value of normalizeImageUrls(values)) {
    addAvailableImage(images, seen, value, source);
  }
}

function normalizeFeatureImageUrl(url) {
...
function normalizeTestimonialText(value) {
  const testimonial = typeof value === 'string' ? value.trim() : '';
  return testimonial.slice(0, 1200);
}
...
export async function addClayPlayEntryLike({ entryId, name, count = 1 }) {
  await ensureInitialized();
  ...
  const likeCount = Math.max(1, Number.isInteger(Number(count)) ? Number(count) : 1);
  if (!personName) {
  ...
  await sql`
    ...
    SELECT ${resolvedEntryId}, ${personName}
    FROM generate_series(1, ${likeCount})
  `;
...
export async function listBookReviewEntryReactions(entryIds = []) {
  await ensureInitialized();
  const ids = Array.from(new Set(entryIds.map((id) => Number(id)).filter((id) => Number.isInteger(id) && id > 0)));
  if (ids.length === 0) return {};
  ...
  const byEntry = {};
  for (const id of ids) {
    byEntry[id] = { likesCount: 0 };
  }
  for (const row of likesRows) {
    if (!byEntry[row.entryId]) byEntry[row.entryId] = { likesCount: 0 };
    byEntry[row.entryId].likesCount = row.likesCount;
  }
...
export async function addBookReviewEntryLike({ entryId, name, count = 1 }) {
  await ensureInitialized();
  const resolvedEntryId = Number(entryId);
  let personName = normalizePersonName(name);
  if (!Number.isInteger(resolvedEntryId) || resolvedEntryId <= 0) throw new Error('Invalid entry id.');
  ...
  await sql`
    INSERT INTO book_reviews_entry_likes (entry_id, person_name)
    SELECT ${resolvedEntryId}, ${personName}
    FROM generate_series(1, ${likeCount})
  `;
...
async function ensureProjectCommentsTables() {
  if (!projectCommentsTablesPromise) {
    projectCommentsTablesPromise = (async () => {
      await ensureProjectsTable();
      await sql`
        CREATE TABLE IF NOT EXISTS project_comments (
          id SERIAL PRIMARY KEY,
      ...
      await sql`ALTER TABLE project_comments ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'pending'`;
      await sql`ALTER TABLE project_comments ADD COLUMN IF NOT EXISTS commenter_token TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE project_comments ADD COLUMN IF NOT EXISTS avatar_url TEXT NOT NULL DEFAULT ''`;
      await sql`
        CREATE TABLE IF NOT EXISTS project_comment_likes (
          id SERIAL PRIMARY KEY,
      ...
      projectCommentsTablesPromise = null;
      throw error;
    });
  }
  await projectCommentsTablesPromise;
}

export async function listProjectComments(projectEntryId) {
  return listProjectCommentsForAdmin(projectEntryId, { includePending: false });
}
...
export async function listProjectCommentsForAdmin(projectEntryId, { includePending = true, commenterToken = '' } = {}) {
  await ensureInitialized();
  ...
  const comments = await sql`
    SELECT id, project_entry_id AS "projectEntryId", parent_comment_id AS "parentCommentId",
           person_name AS "name", comment_text AS "comment", avatar_url AS "avatarUrl",
    ...
    WHERE project_entry_id = ${id}
      AND (
        ...
        OR status = 'green'
        OR (${String(commenterToken || '')} <> '' AND commenter_token = ${String(commenterToken || '')} AND status = 'pending')
      )
    ORDER BY created_at DESC, id DESC
  `;
  if (comments.length === 0) return [];

  return comments.map((row) => normalizeCommentRow(row));
}

export async function addProjectComment({ projectEntryId, name, comment, parentCommentId, commenterToken }) {
  await ensureInitialized();
  await ensureProjectCommentsTables();
  ...
  const safeToken = String(commenterToken || '').trim().slice(0, 120);
  ...
  const rows = await sql`
    INSERT INTO project_comments (project_entry_id, parent_comment_id, person_name, comment_text, status, commenter_token)
    VALUES (${resolvedEntryId}, ${validParentId}, ${personName}, ${commentText}, 'pending', ${safeToken})
    RETURNING id, project_entry_id AS "projectEntryId", parent_comment_id AS "parentCommentId",
              person_name AS "name", comment_text AS "comment", avatar_url AS "avatarUrl",
              created_at AS "createdAt", status
  `;
  if (!rows[0]) return null;
  return normalizeCommentRow(rows[0]);
}

export async function addProjectCommentReplyForAdmin({ projectEntryId, parentCommentId, comment, makeParentGreen = false }) {
  await ensureInitialized();
  await ensureProjectCommentsTables();
  const resolvedEntryId = Number(projectEntryId);
  const resolvedParentId = Number(parentCommentId);
  const replyText = typeof comment === 'string' ? comment.trim().slice(0, 800) : '';
  if (!Number.isInteger(resolvedEntryId) || resolvedEntryId <= 0) throw new Error('Invalid project entry id.');
  if (!Number.isInteger(resolvedParentId) || resolvedParentId <= 0) throw new Error('Invalid parent comment id.');
  ...
  if (makeParentGreen) {
    await sql`
      ...
      WHERE id = ${resolvedParentId}
        AND project_entry_id = ${resolvedEntryId}
    `;
  }

  const rows = await sql`
    INSERT INTO project_comments (project_entry_id, parent_comment_id, person_name, comment_text, status, avatar_url, commenter_token)
    VALUES (${resolvedEntryId}, ${resolvedParentId}, 'ARIVERSE', ${replyText}, 'green', ${HOME_HERO_IMAGE_URL}, '')
    RETURNING id, project_entry_id AS "projectEntryId", parent_comment_id AS "parentCommentId",
              person_name AS "name", comment_text AS "comment", avatar_url AS "avatarUrl",
              created_at AS "createdAt", status
  `;
...
export async function updateOwnProjectPendingComment({ commentId, projectEntryId, comment, commenterToken }) {
  await ensureInitialized();
  await ensureProjectCommentsTables();
  const resolvedCommentId = Number(commentId);
  const resolvedProjectEntryId = Number(projectEntryId);
  const safeToken = String(commenterToken || '').trim().slice(0, 120);
  const nextComment = typeof comment === 'string' ? comment.trim().slice(0, 800) : '';
  if (!Number.isInteger(resolvedCommentId) || !Number.isInteger(resolvedProjectEntryId) || resolvedCommentId <= 0 || resolvedProjectEntryId <= 0) return null;
  if (!safeToken || !nextComment) return null;
  const rows = await sql`
    UPDATE project_comments
    SET comment_text = ${nextComment}
    WHERE id = ${resolvedCommentId}
      AND project_entry_id = ${resolvedProjectEntryId}
      AND status = 'pending'
      AND commenter_token = ${safeToken}
    RETURNING id, project_entry_id AS "projectEntryId", parent_comment_id AS "parentCommentId",
              person_name AS "name", comment_text AS "comment", avatar_url AS "avatarUrl",
              created_at AS "createdAt", status
  `;
...
export async function deleteOwnProjectPendingComment({ commentId, projectEntryId, commenterToken }) {
  await ensureInitialized();
  await ensureProjectCommentsTables();
  const resolvedCommentId = Number(commentId);
  const resolvedProjectEntryId = Number(projectEntryId);
  const safeToken = String(commenterToken || '').trim().slice(0, 120);
  if (!Number.isInteger(resolvedCommentId) || !Number.isInteger(resolvedProjectEntryId) || resolvedCommentId <= 0 || resolvedProjectEntryId <= 0) return;
  if (!safeToken) return;
  await sql`
    DELETE FROM project_comments
    WHERE id = ${resolvedCommentId}
...
export async function updateProjectCommentForAdmin({ commentId, projectEntryId, comment, status }) {
  await ensureInitialized();
  await ensureProjectCommentsTables();
  const resolvedCommentId = Number(commentId);
  const resolvedProjectEntryId = Number(projectEntryId);
  if (!Number.isInteger(resolvedCommentId) || resolvedCommentId <= 0) throw new Error('Invalid comment id.');
  if (!Number.isInteger(resolvedProjectEntryId) || resolvedProjectEntryId <= 0) throw new Error('Invalid project entry id.');

  const nextComment = typeof comment === 'string' ? comment.trim().slice(0, 800) : null;
  const nextStatus = status === 'green' || status === 'pending' ? status : null;
  if (nextComment === null && nextStatus === null) throw new Error('No valid admin update provided.');

  const rows = await sql`
    UPDATE project_comments
    SET
      comment_text = COALESCE(${nextComment}, comment_text),
...
async function ensureContentCommentsTables() {
  if (!contentCommentsTablesPromise) {
    contentCommentsTablesPromise = (async () => {
      await ensureInitialized();
      await sql`
      ...
      `;
      await sql`ALTER TABLE content_comments ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'pending'`;
      await sql`ALTER TABLE content_comments ADD COLUMN IF NOT EXISTS commenter_token TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE content_comments ADD COLUMN IF NOT EXISTS avatar_url TEXT NOT NULL DEFAULT ''`;
      await sql`
      ...
      contentCommentsTablesPromise = null;
      throw error;
    });
  }
  await contentCommentsTablesPromise;
}

async function ensureContentEntryLikesTables() {
  if (!contentEntryLikesTablesPromise) {
    contentEntryLikesTablesPromise = (async () => {
      await ensureInitialized();
      await sql`
        CREATE TABLE IF NOT EXISTS content_entry_likes (
          id SERIAL PRIMARY KEY,
          section_key TEXT NOT NULL,
          entry_id INTEGER NOT NULL,
          person_name TEXT NOT NULL DEFAULT '',
      ...
      `;
      await sql`
        CREATE INDEX IF NOT EXISTS idx_content_entry_likes_section_entry
          ON content_entry_likes(section_key, entry_id);
      `;
    })().catch((error) => {
      contentEntryLikesTablesPromise = null;
      throw error;
    });
  }
  await contentEntryLikesTablesPromise;
}

async function ensureTestimonialsTables() {
  if (!testimonialsTablesPromise) {
    testimonialsTablesPromise = (async () => {
      await ensureInitialized();
      await sql`
      ...
      `;
      await sql`ALTER TABLE testimonials ADD COLUMN IF NOT EXISTS relation_text TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE testimonials ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'pending'`;
      await sql`
        CREATE INDEX IF NOT EXISTS idx_testimonials_status_created_at
          ON testimonials(status, created_at DESC, id DESC);
      `;
    })().catch((error) => {
      testimonialsTablesPromise = null;
      throw error;
    });
  }
  await testimonialsTablesPromise;
}

async function ensureFeatureImagesTables() {
  if (!featureImagesTablesPromise) {
    featureImagesTablesPromise = (async () => {
      await ensureInitialized();
      await sql`
      ...
      `;
      await sql`ALTER TABLE feature_images ADD COLUMN IF NOT EXISTS image_url TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE feature_images ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT now()`;
      await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_feature_images_image_url ON feature_images(image_url)`;
    })().catch((error) => {
      featureImagesTablesPromise = null;
      throw error;
    });
  }
  await featureImagesTablesPromise;
}

export async function listContentComments({ sectionKey, entryId }) {
  return listContentCommentsForAdmin({ sectionKey, entryId, includePending: false });
}

export async function listContentCommentsForAdmin({ sectionKey, entryId, includePending = true, commenterToken = '' }) {
...
export async function listContentEntryReactions({ sectionKey, entryIds = [] }) {
  await ensureContentEntryLikesTables();
  const safeSection = String(sectionKey || '').trim().toLowerCase();
  const ids = Array.from(new Set(entryIds.map((id) => Number(id)).filter((id) => Number.isInteger(id) && id > 0)));
  if (!safeSection || ids.length === 0) return {};

  const likesRows = await sql`
    SELECT entry_id AS "entryId", COUNT(*)::int AS "likesCount"
    FROM content_entry_likes
    WHERE section_key = ${safeSection} AND entry_id = ANY(${ids}::int[])
    GROUP BY entry_id
  `;
...
export async function addContentEntryLike({ sectionKey, entryId, name = '', count = 1 }) {
  await ensureContentEntryLikesTables();
  const safeSection = String(sectionKey || '').trim().toLowerCase();
  const resolvedEntryId = Number(entryId);
  const likeCount = Number(count || 0);
  const safeName = normalizePersonName(name);
  if (!safeSection || !Number.isInteger(resolvedEntryId) || resolvedEntryId <= 0 || !Number.isInteger(likeCount) || likeCount <= 0) {
    return { likesCount: 0 };
  }

  await sql`
    INSERT INTO content_entry_likes (section_key, entry_id, person_name)
    SELECT ${safeSection}, ${resolvedEntryId}, ${safeName}
    FROM generate_series(1, ${likeCount})
  `;
  ...
  const countRows = await sql`
    SELECT COUNT(*)::int AS count
    FROM content_entry_likes
    WHERE section_key = ${safeSection} AND entry_id = ${resolvedEntryId}
  `;
  return { likesCount: countRows[0]?.count || 0 };
}

export async function addContentComment({ sectionKey, entryId, name, comment, parentCommentId, commenterToken }) {
  await ensureContentCommentsTables();
  const safeSection = String(sectionKey || '').trim().toLowerCase();
  ...
  const rows = await sql`
    INSERT INTO content_comments (section_key, entry_id, parent_comment_id, person_name, comment_text, status, commenter_token)
    VALUES (${safeSection}, ${entryId}, ${safeParent}, ${safeName}, ${safeComment}, 'pending', ${safeToken})
    RETURNING id, section_key AS "sectionKey", entry_id AS "entryId", parent_comment_id AS "parentCommentId",
              person_name AS name, comment_text AS comment, avatar_url AS "avatarUrl",
              created_at AS "createdAt", status
  `;
  return rows[0] ? normalizeCommentRow(rows[0]) : null;
}

export async function addContentCommentReplyForAdmin({ sectionKey, entryId, parentCommentId, comment, makeParentGreen = false }) {
  await ensureContentCommentsTables();
  const safeSection = String(sectionKey || '').trim().toLowerCase();
  const resolvedEntryId = Number(entryId);
  const resolvedParentId = Number(parentCommentId);
  const replyText = typeof comment === 'string' ? comment.trim().slice(0, 800) : '';
  if (!safeSection || !Number.isInteger(resolvedEntryId) || resolvedEntryId <= 0) throw new Error('Invalid content comment identifiers.');
  if (!Number.isInteger(resolvedParentId) || resolvedParentId <= 0) throw new Error('Invalid parent comment id.');
  if (!replyText) throw new Error('Reply is required.');
  ...
  if (makeParentGreen) {
    await sql`
      ...
      WHERE id = ${resolvedParentId}
        ...
        AND entry_id = ${resolvedEntryId}
    `;
  }

  const rows = await sql`
    INSERT INTO content_comments (section_key, entry_id, parent_comment_id, person_name, comment_text, status, avatar_url, commenter_token)
    VALUES (${safeSection}, ${resolvedEntryId}, ${resolvedParentId}, 'ARIVERSE', ${replyText}, 'green', ${HOME_HERO_IMAGE_URL}, '')
    RETURNING id, section_key AS "sectionKey", entry_id AS "entryId", parent_comment_id AS "parentCommentId",
              person_name AS name, comment_text AS comment, avatar_url AS "avatarUrl",
              created_at AS "createdAt", status
  `;
...
export async function updateOwnContentPendingComment({ sectionKey, entryId, commentId, comment, commenterToken }) {
  await ensureContentCommentsTables();
  const safeSection = String(sectionKey || '').trim().toLowerCase();
  const resolvedCommentId = Number(commentId);
  const safeToken = String(commenterToken || '').trim().slice(0, 120);
  const nextComment = typeof comment === 'string' ? comment.trim().slice(0, 800) : '';
  if (!safeSection || !Number.isInteger(entryId) || !Number.isInteger(resolvedCommentId) || entryId <= 0 || resolvedCommentId <= 0) return null;
  if (!safeToken || !nextComment) return null;
  const rows = await sql`
    UPDATE content_comments
    SET comment_text = ${nextComment}
...
export async function deleteOwnContentPendingComment({ sectionKey, entryId, commentId, commenterToken }) {
  await ensureContentCommentsTables();
  const safeSection = String(sectionKey || '').trim().toLowerCase();
  const resolvedCommentId = Number(commentId);
  const safeToken = String(commenterToken || '').trim().slice(0, 120);
  if (!safeSection || !Number.isInteger(entryId) || !Number.isInteger(resolvedCommentId) || entryId <= 0 || resolvedCommentId <= 0) return;
  if (!safeToken) return;
  await sql`
    DELETE FROM content_comments
    WHERE id = ${resolvedCommentId}
...
export async function updateContentCommentForAdmin({ sectionKey, entryId, commentId, comment, status }) {
  await ensureContentCommentsTables();
  const safeSection = String(sectionKey || '').trim().toLowerCase();
  const resolvedCommentId = Number(commentId);
  const nextComment = typeof comment === 'string' ? comment.trim().slice(0, 800) : null;
  const nextStatus = status === 'green' || status === 'pending' ? status : null;
  if (!safeSection || !Number.isInteger(entryId) || entryId <= 0 || !Number.isInteger(resolvedCommentId) || resolvedCommentId <= 0) {
    throw new Error('Invalid content comment identifiers.');
  }
  if (nextComment === null && nextStatus === null) throw new Error('No valid admin update provided.');
...
export async function listTestimonialsForAdmin({ includePending = true } = {}) {
  await ensureInitialized();
  await ensureTestimonialsTables();

  const rows = await sql`
    SELECT id,
           person_name AS name,
           relation_text AS relation,
           testimonial_text AS testimonial,
           status,
...
export async function addTestimonial({ name, relation, testimonial }) {
  await ensureInitialized();
  await ensureTestimonialsTables();
  const safeName = normalizePersonName(name) || 'anonymous';
  const safeRelation = typeof relation === 'string' ? relation.trim().slice(0, 80) : '';
  const safeTestimonial = normalizeTestimonialText(testimonial);
  if (!safeTestimonial) return null;

  const rows = await sql`
    INSERT INTO testimonials (person_name, relation_text, testimonial_text, status)
    VALUES (${safeName}, ${safeRelation}, ${safeTestimonial}, 'pending')
    RETURNING id, person_name AS name, relation_text AS relation, testimonial_text AS testimonial,
...
export async function updateTestimonialForAdmin({ id, name, relation, testimonial, status }) {
  await ensureInitialized();
  await ensureTestimonialsTables();
  const resolvedId = Number(id);
  if (!Number.isInteger(resolvedId) || resolvedId <= 0) throw new Error('Invalid testimonial id.');

  const nextName = typeof name === 'string' ? normalizePersonName(name) : null;
  const nextRelation = typeof relation === 'string' ? relation.trim().slice(0, 80) : null;
  const nextTestimonial = typeof testimonial === 'string' ? normalizeTestimonialText(testimonial) : null;
  const nextStatus = status === 'green' || status === 'pending' ? status : null;
  if (nextName === null && nextRelation === null && nextTestimonial === null && nextStatus === null) {
    throw new Error('No valid admin update provided.');
  }

  const rows = await sql`
    UPDATE testimonials
    SET
      person_name = COALESCE(${nextName}, person_name),
      relation_text = COALESCE(${nextRelation}, relation_text),
      testimonial_text = COALESCE(${nextTestimonial}, testimonial_text),
      status = COALESCE(${nextStatus}, status)
    WHERE id = ${resolvedId}
    RETURNING id, person_name AS name, relation_text AS relation, testimonial_text AS testimonial,
              status, created_at AS "createdAt"
...
export async function listFeatureImages() {
  await ensureInitialized();
  await ensureFeatureImagesTables();
...
export async function addFeatureImage({ imageUrl }) {
  await ensureInitialized();
  await ensureFeatureImagesTables();
  const safeImageUrl = normalizeFeatureImageUrl(imageUrl);
  if (!safeImageUrl) return null;

  await sql`
    INSERT INTO feature_images (image_url)
    VALUES (${safeImageUrl})
...
export async function listAllCommentApprovals() {
  await ensureInitialized();
  await ensureProjectCommentsTables();
  await ensureContentCommentsTables();
  ...
  const projectRows = await sql`
    SELECT c.id,
           ...
           p.title AS "entryTitle"
    FROM project_comments c
    LEFT JOIN projects_entries p ON p.id = c.project_entry_id
  ...
  return [
    ...projectRows.map((row) => normalizeCommentRow(row)),
    ...contentRows.map((row) => normalizeCommentRow(row)),
  ].sort((a, b) => {
...
export async function getAdminPendingModerationCount() {
  await ensureInitialized();
  await ensureProjectCommentsTables();
  await ensureContentCommentsTables();
  await ensureTestimonialsTables();

  const [projectRows, contentRows, testimonialRows] = await Promise.all([
    sql`
      SELECT COUNT(*)::int AS count
      FROM project_comments
  ...
  return (projectRows[0]?.count || 0) + (contentRows[0]?.count || 0) + (testimonialRows[0]?.count || 0);
}
