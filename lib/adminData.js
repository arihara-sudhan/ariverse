...
export async function addLinkItem({ linkId, imageUrl, imageUrls, youtubeUrl, markdownText, kavithaiFrom, imageAlign, category, subcategory, companyLogoUrl, subtitle, dateText }) {
  await ensureInitialized();
  const label = await getSectionLabel(linkId);
  if (label === 'Mini-Projects') {
    await ensureMiniProjectsTable();
...
export async function updateLinkItem({ id, linkId, imageUrl, imageUrls, youtubeUrl, markdownText, kavithaiFrom, imageAlign, category, subcategory, companyLogoUrl, subtitle, dateText }) {
  await ensureInitialized();
  const resolved = await resolveItemTableById(id, linkId);
  if (!resolved) return;
