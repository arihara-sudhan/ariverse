...
const LINK_FIX_VERSION = '2026-05-30-ari-xperiments-link-v7';
  ...
  if (label === 'Experiments') {
    ...
    if (rows.length === 0) {
      ...
      for (const item of fallbackAriTrials) {
        await sql`
          INSERT INTO experiments_entries (link_id, image_url, markdown_text, entry_title, read_more_url, sort_order)
          VALUES (${linkId}, ${item.imageUrl || ''}, ${item.description || ''}, ${item.title || ''}, ${item.readMoreUrl || ''}, ${sortOrder})
        `;
        sortOrder += 1;
      }
      rows = await sql`
...
export async function addLinkItem({ linkId, imageUrl, imageUrls, youtubeUrl, markdownText, kavithaiFrom, imageAlign, category, subcategory, companyLogoUrl, subtitle, dateText, bigDescription, projectTags }) {
  ...
  if (label === 'Experiments') {
    ...
    const rows = await sql`
      INSERT INTO experiments_entries (link_id, image_url, markdown_text, entry_title, read_more_url, sort_order)
      VALUES (${linkId}, ${imageUrl || ''}, ${markdownText || ''}, ${kavithaiFrom || ''}, ${youtubeUrl || ''}, ${(maxExperiments[0]?.max_order || 0) + 1})
...
export async function updateLinkItem({ id, linkId, imageUrl, imageUrls, youtubeUrl, markdownText, kavithaiFrom, imageAlign, category, subcategory, companyLogoUrl, subtitle, dateText, bigDescription, projectTags }) {
  ...
  if (resolved.table === 'experiments_entries') {
    await sql`UPDATE experiments_entries SET image_url = ${imageUrl || ''}, markdown_text = ${markdownText || ''}, entry_title = ${kavithaiFrom || ''}, read_more_url = ${youtubeUrl || ''} WHERE id = ${id} AND link_id = ${resolved.linkId}`;
    return;
  }
  if (resolved.table === 'ariyin_kavithaigal') {
