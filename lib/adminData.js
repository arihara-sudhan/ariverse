...
const defaultLinks = [
  ...
  { label: 'Arichuvadu', href: '/arichuvadu', category: 'PASSIONAL' },
...
const DB_INIT_VERSION = '2026-06-11-arichuvadu-seed-v1';
...
async function ensureInitialized({ background = false } = {}) {
  ...
  if (!initPromise) {
    ...
    initPromise = (async () => {
      ...
      await sql`DELETE FROM profile_links WHERE label = 'அரிச்சுவடி' OR href = '/arichuvadi'`;
      ...
      await sql`
        ...
        WHERE label IN ('Experiments', 'My Books', 'Shelf', 'Thirukkural', 'For AI', 'Guest Lectures', 'AI with ARI (YouTube)', 'Arichuvadu')
      `;
      ...
      const existingArichuvadu = await sql`SELECT id FROM profile_links WHERE label = 'Arichuvadu' LIMIT 1`;
      if (!existingArichuvadu[0]) {
        const maxArichuvadu = await sql`SELECT COALESCE(MAX(sort_order), 0)::int AS max_order FROM profile_links`;
        await sql`
          INSERT INTO profile_links (label, href, category, sort_order, is_hidden)
          VALUES ('Arichuvadu', '/arichuvadu', 'PASSIONAL', ${(maxArichuvadu[0]?.max_order || 0) + 1}, 0)
        `;
      }
...
export async function getResumeAssets(linkId) {
  ...
  const rows = await sql`
    SELECT link_id AS "linkId", pdf_url AS "pdfUrl", page_image_urls AS "pageImageUrlsRaw", updated_at AS "updatedAt"
    FROM resume_assets
...
export async function upsertResumeAssets({ linkId, pdfUrl, pageImageUrls }) {
  await ensureInitialized();
  const normalizedImages = normalizeImageUrls(pageImageUrls);
  const rows = await sql`
    INSERT INTO resume_assets (link_id, pdf_url, page_image_urls, updated_at)
    VALUES (${linkId}, ${pdfUrl || ''}, ${JSON.stringify(normalizedImages)}, now())
    ON CONFLICT (link_id)
    DO UPDATE SET
      pdf_url = EXCLUDED.pdf_url,
      page_image_urls = EXCLUDED.page_image_urls,
      updated_at = now()
    RETURNING link_id AS "linkId", pdf_url AS "pdfUrl", page_image_urls AS "pageImageUrlsRaw", updated_at AS "updatedAt"
  `;
  return {
    ...rows[0],
    pageImageUrls: normalizeImageUrls(rows[0].pageImageUrlsRaw),
