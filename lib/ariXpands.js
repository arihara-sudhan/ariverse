...
function normalizeXpandRow(row = {}) {
  return {
    ...
    curriculumUrl: normalizeOptionalText(row?.curriculum_url || row?.curriculumUrl, 1000),
...
async function ensureSchema() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS ari_xpands (
      ...
      curriculum_url TEXT NOT NULL DEFAULT '',
  ...
  `;
  await sql`ALTER TABLE ari_xpands ADD COLUMN IF NOT EXISTS curriculum_url TEXT NOT NULL DEFAULT ''`;
...
export async function createXpand(input = {}) {
  ...
  const rows = await sql`
    INSERT INTO ari_xpands (
      title, slug, subtitle, description, status, visibility, cover_image, curriculum_url, start_date, end_date, tags, created_at, updated_at, archived_at
    )
    VALUES (
      ${title},
      ${slug},
      ...
      ${normalizeNullableUrl(input.curriculumUrl || input.curriculum_url)},
...
export async function updateXpand(id, input = {}) {
  ...
  const rows = await sql`
    ...
    SET
      ...
      curriculum_url = ${normalizeNullableUrl(input.curriculumUrl ?? input.curriculum_url ?? existing.curriculumUrl)},
