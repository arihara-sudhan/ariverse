...
function shouldPreservePoemLineBreaks(baseFolder, options = {}) {
  const normalizedFolder = cleanText(baseFolder);
  const normalizedCategorySlug = normalizeSlug(options.categorySlug || '');
  const normalizedSlug = normalizeSlug(options.slug || '');

  return (
    normalizedCategorySlug === KAVITHAIGAL_CATEGORY_SLUG ||
    normalizedSlug.startsWith('kavithai-') ||
...
function markdownToHtml(content, baseFolder, options = {}) {
  const normalizedFolder = cleanText(baseFolder);
  const preserveLineBreaks = shouldPreservePoemLineBreaks(normalizedFolder, options);
...
async function loadMarkdownFromContentPath(contentPath) {
  const normalizedPath = normalizePathValue(contentPath);
  if (!normalizedPath) return '';
  try {
    const response = await fetch(toPublicStorageUrl(normalizedPath));
...
export async function getArichuvadiPostBySlug(slug) {
  ...
  let markdown = '';
  if (post.contentPath) {
    markdown = await loadMarkdownFromContentPath(post.contentPath);
  }
  if (!markdown && typeof row.content_markdown === 'string' && row.content_markdown.trim()) {
