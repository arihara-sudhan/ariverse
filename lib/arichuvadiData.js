...
export async function getArichuvadiPostBySlug(slug) {
  ...
  const fallbackRow = fallbackRows.find((row) => {
    ...
    return belongsToArichuvadi(post) && (post.slug === normalizedSlug || normalizeSlug(post.title) === normalizedSlug);
  }) || null;
  if (!fallbackRow) return null;
  const fallbackMatch = normalizePostRow(fallbackRow, categoryMap);

  let markdown = '';
  if (typeof fallbackRow.content_markdown === 'string' && fallbackRow.content_markdown.trim()) {
    markdown = fallbackRow.content_markdown;
  } else if (fallbackMatch.contentPath) {
    const contentUrl = toPublicStorageUrl(fallbackMatch.contentPath);
  ...
  const html = markdownToHtml(markdown, fallbackMatch.storageFolder);

  return {
    ...fallbackMatch,
    content: markdown,
    title: extractMarkdownTitle(markdown) || fallbackMatch.title,
    html,
  };
}
