...
export function enforceSameOriginWrite(req, res) {
  ...
  if (host && (fetchSite === 'same-origin' || fetchSite === 'same-site')) {
    return true;
  }
...
export function checkRateLimit(req, keyPrefix, maxHits, windowMs) {
  return incrementRateLimit(req, keyPrefix, maxHits, windowMs);
}

export function getRateLimitStatus(req, keyPrefix, maxHits, windowMs) {
...
export function incrementRateLimit(req, keyPrefix, maxHits, windowMs) {
  const ip = getClientIp(req);
  const key = `${keyPrefix}:${ip}`;
  const current = RATE_LIMIT_BUCKETS.get(key);
  const now = nowMs();
...
export function clearRateLimit(req, keyPrefix) {
  const ip = getClientIp(req);
  RATE_LIMIT_BUCKETS.delete(`${keyPrefix}:${ip}`);
}
